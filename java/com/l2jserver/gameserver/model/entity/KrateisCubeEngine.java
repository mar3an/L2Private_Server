...
public class KrateisCubeEngine
{
	private static final Logger _log = Logger.getLogger(KrateisCubeEngine.class.getName());
	...
	private void countDownEndMessage()
	{
		...
		{
			_log.log(Level.WARNING, getClass().getSimpleName() + ": Error in count down end message: " + e.getMessage(), e);
		}
	}
	...
	private void countDownStartMessage()
	{
		...
		{
			_log.log(Level.WARNING, getClass().getSimpleName() + ": Error in count down start message: " + e.getMessage(), e);
		}
	}
	...
	private void startKrateisCube()
	{
		...
		if (Config.KRATEIS_CUBE_EVENT_LOG_ENABLED)
		{
			_log.info("Krateis Cube: Match initiated!");
		}
	}
